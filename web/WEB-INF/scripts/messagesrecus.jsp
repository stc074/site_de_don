...
<html>
<head>
<title>Don et recupe - Site de dons - Messages reçus</title>
           ...
           try {
               if(request.getAttribute("info")!=null) {
                   ...
                   switch(info) {
                       ...
                       Objet.getConnection();
                       String query="SELECT t1.id,t1.titre,t1.timestamp,t1.etat,t2.titre AS titreAnnonce,t3.pseudo FROM table_messages AS t1,table_annonces AS t2,table_membres AS t3 WHERE t2.id=t1.id_annonce AND t3.id=t1.id_membre_expediteur AND t1.id_membre_destinataire=? ORDER BY t1.timestamp DESC";
                       ...
                       while(result.next()) {
                           ...
                           %>
                           <div class="cadre2">
                               <a href="./message-recu-<%= idMessage %>.html" title="CONSULTER CE MESSAGE" rel="nofollow"><%= titre %></a>
                               <%
                               if(etat==0) { %>
                               <span>[<span class="clign">Non lu</span>]</span>
                               <% } %>
                               <span>&nbsp;Envoyé le <%= cal.get(cal.DATE) %>-<%= cal.get(cal.MONTH)+1 %>-<%= cal.get(cal.YEAR) %> par <%= pseudo %>,</span>
                               <span>&nbsp;[Annonce :: <%= titreAnnonce %>]</span>
                               <span>&rArr;<a href="efface-message-1-<%= idMessage %>.html" title="ÉFFACER CE MESSAGE" rel="nofollow">Éffacer</a></span>
                           </div>
                           <br/>
                           <%
                           }
